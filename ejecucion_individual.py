...
from utilidades import (
    exportar_ruta_meshlab,
    exportar_grafo_meshlab,
    visualizar_ruta_3d_con_mesh
)

def obtener_parametros_escenario(escenario):
    if escenario == 1:
        return {
            "d_f": 0.15,
            "R_1": 0.02,
...
def ejecutar_algoritmo(algoritmo, dist_matrix, seed, verbose=False, ciclo_cerrado=False):
    n_nodos = dist_matrix.shape[0]

    if algoritmo == "ACO_AS":
        return aplicar_ant_system(
        ...
        return aplicar_elitist_ant_system(
            dist_matrix=dist_matrix,
            n_hormigas=n_nodos,
            n_iteraciones=100,
            alpha=1.0,
        ...
        return aplicar_min_max_ant_system(
            dist_matrix=dist_matrix,
            n_hormigas=n_nodos,
            n_iteraciones=100,
            alpha=1.0,
        ...
        return aplicar_ant_colony_system(
            dist_matrix=dist_matrix,
            n_hormigas=10,
            n_iteraciones=100,
            alpha=1.0,
        ...
        return aplicar_simulated_annealing(
            dist_matrix=dist_matrix,
            n_iteraciones=10000,
...
if __name__ == "__main__":
    """
    Dado un escenario y un algoritmo, crea los viewpoints, el grafo, y ejecuta el algoritmo
    ...
    path_grafo = exportar_grafo_meshlab(
        viewpoints=viewpoints,
        dist_matrix=dist_matrix,
        output_dir=output_dir,
        nombre_grafo=f"grafo_escenario{escenario}_k{parametros_escenario['k_vecinos']}.ply",
